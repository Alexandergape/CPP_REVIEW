...
#ifndef FIRST_STEPS_WITH_C___BINARYSEARCHTREE_H
#define FIRST_STEPS_WITH_C___BINARYSEARCHTREE_H
...
public:
    BinarySearchTree(): Size(0), root(nullptr) {};// = default;

    ~BinarySearchTree() {
        // cout << endl << "deleting me ... " << endl;
        // while (!isEmpty())
        // {
...
//        node->setElement(newValue);
    ...
    vector<Node<E>*> children(Node<E> &node) const{
//        auto children=new vector<E>();
        vector<Node<E>*> children;
...
private:
    ...
    void postOrder(Node<E> &node, vector<Node<E> *> &nodes) const {
        vector<Node<E>*> a=children(node);
        for (Node<E> *child : a)
            postOrder(*child, nodes);
        nodes.push_back(&node); // for postOrder, we add position p after exploring subtrees
    }
...
#endif //FIRST_STEPS_WITH_C___BINARYSEARCHTREE_H
